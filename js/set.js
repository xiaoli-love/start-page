...
由 LiCheng 修改
github:https://github.com/LiCheng99
主页：https://www.lcclc.cn/
日期：2023-03-04
*/
...
function setBgImgInit() {
	...
	switch (bg_img["type"]) {
		case "1":
			...
			break;
		case "2":
			$('#bg').attr('src', 'https://myhkw.cn/open/img/mm') //随机小姐姐
			break;
		case "3":
			$('#bg').attr('src', 'https://myhkw.cn/open/img/scen') //随机风景
			break;
		case "4":
			$('#bg').attr('src', 'https://myhkw.cn/open/img/acg') //随机二次元
			break;
		case "5":
...
function keywordReminder() {
	...
	if (keyword != "") {
		$.ajax({
			url: 'https://cn.bing.com/search' + keyword,
			dataType: 'jsonp',
			...
			success: function(data) {
				...
				$.each(data.s, function(i, val) {
					$('#keywords').append(
						`<div class="keyword" data-id="${i + 1}"><i class='iconfont icon-sousuo'></i>${val}</div>`
						);
				});
...
function seList() {
	...
	for (var i in se_list) {
		html +=
			`<div class='se-li' data-url='${se_list[i]["url"]}' data-name='${se_list[i]["name"]}' data-icon='${se_list[i]["icon"]}'>
...
$(document).ready(function() {
	...
	$(document).on('click', function(e) {
		// 选择搜索引擎点击
		if ($(".search-engine").is(":hidden") && $(".se").is(e.target) || $(".search-engine").is(
				":hidden") && $("#icon-se").is(e.target)) {
